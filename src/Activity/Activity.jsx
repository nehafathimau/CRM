    ...
    const fetchData = async () => {
        ...
        try {
           ...
           const contacts = Array.isArray(contactsRes) ? [...contactsRes] : [...(contactsRes?.data || contactsRes?.content || contactsRes?.api || [])]
            ...
            // Reverse contacts to show newest first (same as Contacts.jsx)
            contacts.reverse()
...
setPersonOptions(leadOptions)
            ...
            console.log("Processed contacts array (reversed):", contacts)
            if (contacts.length > 0) {
                console.log("Setting latest contact to index 0 (now newest):", contacts[0])
                setLatestContact(contacts[0])
            } else {
    ...
    const handleLogActivity = async (values) => {
        try {
            ...
            const payload = {
                ...
                leadId: parseInt(values.leadId) || null,
                ...
                <div className="bg-white justify-between rounded-2xl p-4 sm:p-5 shadow-sm border border-[#e2e8f0] flex flex-col">
                    <div>
                        ...
                        <div className="space-y-10 my-auto py-6 px-2">
                            ...
                            <ContactInfoItem icon={FiUser} label="Assigned:" value={latestContact?.assignedStaff || '—'} />
                        </div>
                    </div>
                    <button
